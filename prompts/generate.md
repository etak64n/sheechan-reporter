...
6. **まとめ** — 要点を **箇条書き(`-` のリスト)** で 2〜4 個振り返る。
...
- **読みやすさのために適度に改行・段落分けを入れる**。1 段落は 2〜4 文程度に抑え、
  話題が変わったら段落を分ける(空行=Markdown の段落区切りを入れる)。長い一枚岩の
  段落は作らない。箇条書きが向く内容(要点の列挙・手順・項目比較)は `-` の
  リストにする
